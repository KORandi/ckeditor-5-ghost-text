...
import loadingPrompt from '../theme/icons/loading-prompt.svg';
import { debounce } from './utils.js';
import { GhostTextCommand } from './ghosttextcommand.js';
import { GhostTextInsertCommand } from './ghosttextinsertcommand.js';
import { GhostTextLoadingCommand } from './ghosttextloadingcommand.js';
import { GhostTextRemoveCommand } from './ghosttextremovecommand.js';
...
export default class GhostTextEditing extends Plugin {
	...
	private insertWrapper = debounce(
		...
		this.getConfig().debounceDelay || 300
	);
	...
	private createGhostTextView(
		...
		const content = this.applyTextDecoration(
			modelElement.getAttribute( 'data-value' ) as string,
			Object.fromEntries(
	...
	private handleInsertKeystroke( event: Event, cancel: VoidFunction ) {
		event.preventDefault();
		this.insertGhostText();
		cancel();
	}

	private handleAcceptKeystroke( event: Event, cancel: VoidFunction ) {
		if ( this.isLoading || !this.fetchedText ) {
			return;
		}
	...
	private removeGhostLetter( writer: Writer, changes: Array<DiffItem> ): boolean {
	...
	private getInsertedContents(
		...
		return changes.reduce<Array<Item>>( ( acc, change ) => {
	...
	private handleSelectionChange( _: EventInfo, { directChange }: { directChange: boolean } ) {
		const editor = this.editor;
	...
	private handleGhostTextClick( domEvent: Event ) {
	...
	private getConfig() {
		return this.editor.config.get( 'ghostText' ) as EditorConfig['ghostText'];
	}

	private handeInsertGhostTextError( error: Error ) {
